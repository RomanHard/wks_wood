        ...
        static function _do_import()
        {
            if(isset($_REQUEST['th_do_import']) and $_REQUEST['th_do_import'] || isset($_REQUEST['media']))
            {
                ...
                if(!current_user_can('manage_options')){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('You do not have permission to do this','decofurniture')));die;
                }
                self::load_lib();

                //Check Importer Plugins was installed
                if ( !class_exists( 'WP_Importer' ) or ! class_exists( 'WP_Import' ) ){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('Importer Class Was Not Installed','decofurniture')));die;
                }
                global $th_config;
                    ...
                    if(!empty($data_option)){
                        ...
                        if(!function_exists('th_get_option_name'))
                        {
                            echo json_encode( array(
                                    'status'   =>0,
                                    'messenger'=>"<span class='red'>".esc_html__("Plugin: 7up core must be installed first. Stop working!","decofurniture")."</span>",
                                    'next_url' => ''
                                )
                            );
                            die;
                        ...
                        echo json_encode( array(
                                'status'   =>"ok",
                                'messenger'=>esc_html__("Importing the demo theme options...","decofurniture")."<span>".esc_html__("DONE!","decofurniture")."</span><br>",
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("Theme option contain NULL content. Stop working!","decofurniture")."</span>",
                                'next_url' => ''
                            )
                        );
                    }
                ...
                if($step==2){
                    ...
                    if(!empty($data_object)){
                        ...
                        echo json_encode( array(
                                'status'   =>1,
                                'messenger'=>esc_html__("Importing the demo widgets...","decofurniture")." <span>".esc_html__("DONE!","decofurniture")."</span>.<br>",
                                'next_url' => admin_url(self::$_import_page."&th_do_import=1&media={$import_media}&package={$package}&step=" . ($step + 1)),
                            )
                        );
                    }
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("Widget option contain NULL content. Stop working!","decofurniture")."</span>",
                                'next_url' => ''
                            )
                        );
                    }
                ...
                if($step==3){
                    ...
                    if(!$file_name){
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("File Not Found. Stop working!","decofurniture")."</span>",
                                'next_url' => ''
                            )
                        );
                        die;
                ...
                if($step==4) {
                    ...
                    echo json_encode(array(
                            'status' => 1,
                            'messenger' => esc_html__("Importing menu settings ...","decofurniture")." <span>".esc_html__("DONE!","decofurniture")."</span><br>",
                            'next_url' => $nexturl,
                        )
                    );
                }
                ...
                if($step == '5'){
                    ...
                    if($stt_file<count($ds_file) && class_exists('RevSlider')){   
                        ...
                        if(!$set_woocommerce_page) {
                            $next_url = '';
                            $messenger = '<span>'.esc_html__("All Done! Have Fun","decofurniture").'</span>';
                        }
                        else {
                        ...
                        echo json_encode( array(
                                'status'   =>"ok",
                                'messenger'=>esc_html__("Setting reading options...","decofurniture")." <span>".esc_html__("DONE!","decofurniture")."</span><br/>".$messenger,
                                'next_url' => $next_url,
                            )
                        );
                    }
                ...
                if($step=='6')
                {
                    ...
                    echo json_encode( array(
                            'status'   =>"ok",
                            'messenger'=>esc_html__("Setting WooCommerce options...","decofurniture")." <span>".esc_html__("DONE!","decofurniture")."</span><br/><span>".esc_html__("All Done! Have Fun","decofurniture")."</span>",
                            'next_url' => '',
                        )
                    );
                }
