...
if(!function_exists('th_list_post_type')){
    function th_list_post_type($post_type = 'page',$add_empty = false,$convert = false){
        ...
        $page_list = array();
        if($add_empty){
            if(!$convert) $page_list[''] = esc_html__('-- Choose One --','decofurniture');
            else{
                $page_list[] = array(
...
if ( !function_exists( 'th_get_google_link' ) ) {
    function th_get_google_link() {
        ...
        if ( 'off' !== _x( 'on', 'Google font: on or off', 'decofurniture' ) ) {
                ...
                switch ($preload_style) {
                    ...
                    case 'custom-image':
                        ...
                        <div id="loading-center-absolute-image">
                            <img src="<?php echo esc_url($preload_img)?>" alt="<?php esc_attr_e("preload-image","decofurniture");?>"/>
                        </div>
                        <?php
                        break;
...
if(!function_exists('th_get_sidebar_list')){
    function th_get_sidebar_list(){
        ...
        $sidebars = array(
            esc_html__('--Select--','decofurniture') => ''
            );
                            ...
                            switch ($value) {
                                ...
                                case 'tags':
                                    ...
                                    if($tags):?>
                                        <li class="meta-item"><i class="la la-tags" aria-hidden="true"></i>
                                            ...
                                            <?php if($tags) echo apply_filters('th_output_content',$tags); else esc_html_e("No Tag",'decofurniture');?>
                                        </li>
                                        <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
                                    <?php endif;
                                    break;
                                ...
                                case 'comments':
                                    ...
                                    <li class="meta-item"><i aria-hidden="true" class="la la-comment"></i>
                                        ...
                                        <?php 
                                            if(get_comments_number() != 1) esc_html_e('Comments', 'decofurniture') ;
                                            else esc_html_e('Comment', 'decofurniture') ;
                                        ?>
                                        </a>
                                    </li>
                                    <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
                                ...
                                case 'views':
                                    ...
                                    <li class="meta-item"><i class="la la-eye"></i>
                                        ...
                                        if(th_get_post_view() != 1) echo esc_html__("Views",'decofurniture');
                                        else echo esc_html__("View",'decofurniture');
                                        ?>
                                        </span>
                                    </li>
                                    <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
...
if(!function_exists('th_get_post_list_style')){
    function th_get_post_list_style(){
        $list = apply_filters('th_post_list_item_style',array(
            ...
            'style2' => esc_html__('Post list 2','decofurniture'),
            'style3' => esc_html__('Post list 3','decofurniture'),
            'style4' => esc_html__('Post list 4','decofurniture'),
            'style5' => esc_html__('Post list 5','decofurniture'),
            ));        
        return $list;
    }
}
if(!function_exists('th_get_post_style')){
    function th_get_post_style($style = 'element'){
        $list = apply_filters('th_post_item_style',array(
            '' => esc_html__('Default','decofurniture'),
            'style2' => esc_html__('Post grid 2','decofurniture'),
            ));
...
if(!function_exists('th_get_product_list_style')){
    function th_get_product_list_style(){
        $list = apply_filters('th_product_list_item_style',array(
            ''          => esc_html__('Default','decofurniture'),
            'style2'    => esc_html__('Product list 2','decofurniture'),
            ));
        return $list;
    }
}
if(!function_exists('th_get_product_style')){
    function th_get_product_style($style = 'element'){
        $list = apply_filters('th_product_item_style',array(
            ''          => esc_html__('Default','decofurniture'),
            'style2'    => esc_html__('Product grid 2','decofurniture'),
            'style3'    => esc_html__('Product grid 3','decofurniture'),
            'style4'    => esc_html__('Product grid 4','decofurniture'),
            'style5'    => esc_html__('Product grid 5','decofurniture'),
            ));
        return $list;
    }
}
...
if(!function_exists('th_compare_url')){
    function th_compare_url($icon='',$id = false,$text = '',$class='silver'){
        $html = '';
        if(empty($text)) $text = esc_html__("Compare","decofurniture");
...
if(!function_exists('th_wishlist_url')){
    function th_wishlist_url($icon='',$text='',$class='silver'){
        $html = '';
        if(empty($text)) $text = esc_html__("Wishlist","decofurniture");
...
if(!function_exists('th_breadcrumb')){
    function th_breadcrumb($step = '') {
        global $post;
        if(is_home() && !is_front_page()) echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Home','decofurniture').'</a>'.$step.'<span>'.esc_html__('Blog','decofurniture').'</span>';
        else echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Home','decofurniture').'</a>';
        if (is_single()){
        ...
        elseif(is_search()) echo apply_filters('s7upf_output_content',$step)."<span>".esc_html__("Search Results for: ","decofurniture").get_search_query().'</span>';
        elseif(is_404()) echo apply_filters('s7upf_output_content',$step)."<span>".esc_html__("404 ","decofurniture")."</span>";
    }
}
if(!function_exists('th_fix_import_category')){
    function th_fix_import_category($taxonomy){
...
if(!function_exists('th_auto_fix_woocommerce_pages')){
    function th_auto_fix_woocommerce_pages(){
        $data = array(
        ...
        foreach ($data as $key => $item) {
            ...
            if(isset($page->ID)) update_option($key,$page->ID);
        }
    }
}
// fix popup register
if(!function_exists('th_register_user')){
    function th_register_user(){
        ...
        $output = array();
        $output['status'] = 'error';
        $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your username or email has been exist. Please check again. ","decofurniture").'</div></div>';
        if(!empty($user_data)){
            if(!username_exists($user_data['user_login']) && !email_exists($user_data['user_email'])){                
                ...
                $output['message'] = '<div class="message login_info ms-info ms-default">'.esc_html__("Account registration has been successful","decofurniture").'</div>';
            }            
            if(username_exists($username)){
                $output['status'] = 'error';
                $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your username has been exist.","decofurniture").'</div></div>';
            }           
            if(username_exists($email)){
                $output['status'] = 'error';
                $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your email has been exist.","decofurniture").'</div></div>';
            }
        }
        echo ''.$output['message'];
        die();
