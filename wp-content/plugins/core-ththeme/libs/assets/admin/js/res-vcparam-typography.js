(function($) {
	...
	function bb_typo_build_css(){
		var bb_typography_el = $('.th-typography-value');
		...
		bb_typography_el.each(function(index){
			...
			var _self = $(this),
				bb_typo_container = _self.closest('.th-typography');
	...
	$('document').ready(function(){
		$('.hide-color-picker.th-colorpicker-opacity').opacity_wpColorPicker();
		...
		$('.res-title-tab').on('click',function(event){
			...
			$(this).parents('.th-typography').find('.block-wrap-content').fadeOut();
			$(this).parents('.th-typography').find('.block-wrap-content[data-res="'+res_id+'"]').fadeIn();
		})
		$('.th-typography .iris-palette,.wp-picker-clear').on('click', function(){
			setTimeout( bb_typo_build_css, 100);
		});

		$('.th-typography').on('keydown' , 'input', function(){
			setTimeout( bb_typo_build_css, 100);
		});
		
		$('.th-typography').on('change' , 'select', function(){
			setTimeout( bb_typo_build_css, 100);
		});
